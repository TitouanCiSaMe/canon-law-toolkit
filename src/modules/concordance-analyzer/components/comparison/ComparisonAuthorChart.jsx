...
/**
 * Composant ComparisonAuthorChart - Comparaison des auteurs entre 2 corpus
 * 
 * Affiche 2 bar charts côte à côte pour comparer la répartition des auteurs
 * médiévaux entre le corpus A et le corpus B.
 * 
 * @component
 * @param {Object} props - Propriétés du composant
 * @param {Array<{name: string, value: number}>} props.dataA - Auteurs du corpus A
 * @param {Array<{name: string, value: number}>} props.dataB - Auteurs du corpus B
 * @param {number} [props.height=400] - Hauteur en pixels
 * @param {number} [props.maxItems=10] - Nombre maximum d'auteurs affichés
 * @param {string} [props.chartId] - ID unique pour export
 * 
 * @returns {JSX.Element} 2 graphiques côte à côte avec titres
 */
const ComparisonAuthorChart = ({
  dataA,
  dataB,
  height = 400,
  maxItems = 10,
  chartId = `comparison-authors-${Date.now()}`
}) => {
  ...
  return (
    <div id={chartId}>
      ...
      <h3 style={{
        ...
        ✒ {t('concordance.views.corpusComparison.charts.authors.title')}
      </h3>
...
export default ComparisonAuthorChart;
