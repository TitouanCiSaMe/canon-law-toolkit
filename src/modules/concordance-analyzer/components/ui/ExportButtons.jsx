...
/**
 * Composant ExportButtons - Boutons d'export de données et graphiques (Version 2)
 * 
 * VERSION AMÉLIORÉE avec support de sélection de graphique pour vues multi-graphiques.
 * Permet de choisir quel graphique exporter via un menu déroulant.
 * 
 * Nouvelles fonctionnalités (v2) :
 * - Sélecteur de graphique (dropdown)
 * - Support multi-graphiques dans une même vue
 * - Interface cohérente pour toutes les vues
 * 
 * Fonctionnalités conservées :
 * - Export concordances complètes (CSV)
 * - Export statistiques analytiques (JSON)
 * - Export graphique actuel (PNG haute résolution)
 * - Export vue spécifique (CSV filtré)
 * - Activation/désactivation conditionnelle selon les données
 * 
 * @component
 * @param {Object} props - Propriétés du composant
 * @param {Array<Object>} props.filteredData - Données filtrées à exporter
 * @param {Object} props.analytics - Statistiques calculées
 * @param {string} [props.viewType] - Type de vue actuelle
 * @param {string|Array<Object>} [props.chartId] - ID du graphique OU tableau d'options pour sélection
 * @param {string} [props.chartName] - Nom de base pour le fichier PNG (utilisé si chartId est string)
 ...
 * 
 * @returns {JSX.Element} Groupe de boutons d'export avec sélecteur optionnel
 * 
 * @example
 ...
 * 
 * @example
 * // Usage avancé (vue avec plusieurs graphiques) - Nouveau v2
 * <ExportButtons
 *   filteredData={concordances}
 *   analytics={stats}
 *   chartId={[
 *     { id: 'temporal-chart-123', name: 'graphique_agrege', label: 'Graphique agrégé' },
...
const ExportButtons = ({ 
  ...
  // ============================================================================
  // DÉTECTION DU MODE : SIMPLE vs MULTI-GRAPHIQUES
  // ============================================================================
  
  /**
   * Détermine si on est en mode multi-graphiques
   * @type {boolean}
   */
  ...
  /**
   * Graphique sélectionné (pour mode multi-graphiques)
  ...
  /**
   * Gère l'export du graphique en PNG
  ...
  const handleExportChart = () => {
    if (isMultiChart && chartOptions) {
      // Mode multi-graphiques : exporter le graphique sélectionné
      const selected = chartOptions[selectedChartIndex];
      
      // Détecter si c'est la timeline (contenu scrollable)
  ...
  /**
   * Détermine si le bouton PNG doit être affiché
   * @type {boolean}
   */
  ...
  return (
    <div style={{
      ...
      {/* ====================================================================
          BOUTON 3 : EXPORT GRAPHIQUE PNG (AVEC SÉLECTEUR SI MULTI)
          ==================================================================== */}
      {showPNGButton && (
        <div style={{
          display: 'flex',
          ...
          {/* Sélecteur de graphique (affiché uniquement en mode multi) */}
...
/**
 * Composant ExportButton - Bouton d'export individuel stylisé
 * 
 * Identique à la v1, aucun changement
 */
 ...
 * RÉTROCOMPATIBILITÉ :
 * 
 * La v2 est 100% rétrocompatible avec la v1.
 ...
 *   ...
 * />
 * ✅ Active automatiquement le mode multi-graphiques avec sélecteur
 */
 ...
 *   label: string    // Texte affiché dans le sélecteur
 * }
 * 
 * Exemple :
 * {
 ...
 *   label: 'Graphique agrégé'
 * }
 * 
 * Générera : graphique_agrege_2025-11-01.png
 */
